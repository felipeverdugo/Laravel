                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="space-y-10">
                        ...
                        <div>
                                ...
                                <x-jet-input id="dni" class="block mt-1 w-full" type="text" maxlength="8" name="dni" value="{{$user->dni}}" required autofocus autocomplete="dni" disabled readonly/>
                                @if($errors->has('dni'))
                                <p class="mt-2 text-sm text-red-600">{{ $errors->first('dni') }}</p>
                                @endif
                            </div>
                            <div>
                                <x-jet-label for="name" value="Usuario" />
                                <x-jet-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{$user->name}}" required autofocus autocomplete="name" />
                                @if($errors->has('nombre'))
                                <p class="invalid-feedback">{{ $errors->first('nombre') }}</p>
                                @endif
                            </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="flex items-center justify-between">
                                    <label class="flex items-center">
                                        <input name="roles[]" type="radio" class="form-checkbox" value="{{ $role->name }}" {{ $user->getRoleNames()->contains($role->name) ? "checked='checked'":"" }}>
